 ...
 * corporate network from the output of corp.scratch.RunModelTree from the 
 * CorporateRelationExtraction project. Each line of the RunModelTree output
 * contains a JSON object representing an organization mention and a 
 * distribution over types for the relation between the mention and its 
 * author.  These mention objects output by RunModelTree are deserialized 
 * through corp.net.CorpNetEdgeSource (at least in the reducer).
 * 
 * The output produced by HConstructCorpNet contains document lines of the
 * form:
 * 
 * DOC.<Network>.<Press release document name>	<JSON object for the document>
 * 
 * Node lines of the form:
 * 
 * NODE.<Network>.<Node ID>	<JSON object for the node>
 * 
 * And edge lines of the form:
 * 
 * EDGE.<Network>.<Node ID1>.<Node ID2>	<JSON object for edge> \
 * <JSON list of source mentions>
 * 
 * The <Network> specifies the name of the network in which the object is
 * contained.  Currently, separate networks of objects are constructed for
 * each year along with a full network over the whole data set.  The <Node ID>
 * for nodes is generated by the entity resolution function (described in the 
 * technical report).  <Node ID1> and <Node ID2> for a given edge are ordered
 * such that <Node ID1> always occurs alphabetically prior to <Node ID2>.  The
 * JSON objects of each type are represented and deserialized through the 
 * classes in the corp.net package.  See the documentation of those classes for
 * details about the contents of each object.
 * 
